...
public class AckListner implements Runnable {
		...
		while (true) {
			try {
				...
				while (!DataRepository.AckQueue.isEmpty()) {
					...
					if (seqNo == -1) {// if packet with seqno 0 is lost
						...
						if (SlidingWindow.Window.get(seqindex) != null) {
							while (SlidingWindow
								...
								if (tempDg == null) {
									...
									if (DataRepository.WINDOWSIZE == 1) {
										TerminateConnection();
									}
	...
	private void TerminateConnection() {
		...
		System.out.println("Final ACK received, seqNo: "
				+ DataRepository.LAST_DATAPACKET_SEQNO);
		System.out
				.println("Time taken: "
						+ (System.currentTimeMillis() - DataRepository.TimeTakenForFileTransfer)
						/ 1000.0 + " sec");
		SendAckToTerminate();
		System.exit(0);
	}
